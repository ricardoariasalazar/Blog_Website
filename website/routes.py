...
import random, flask , os
from PIL import Image
from website import app, db, bcrypt, mail
from website.forms import RegistrationForm, LoginForm, UpdateAccountForm, PostForm, RequestResetForm, ResetPasswordForm
...
def login():
    ...
    if form.validate_on_submit():
      ...
      if user and bcrypt.check_password_hash(user.password, form.password.data): # Check if exists and the password 
        ...
        return redirect(next_page) if next_page else redirect(url_for('home')) # If next exist redirect them after logging in
      else:
